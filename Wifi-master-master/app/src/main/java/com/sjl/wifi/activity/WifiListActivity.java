...
//            }
    ...
    public void convertScanResult() {
        ...
        if (!CollectionUtils.isNullOrEmpty(scanResults)) {
            for (int i = 0; i < scanResults.size(); i++) {
                ...
                nowList.add(wifiBean);
                if (wifiBean.getWifiName().startsWith(DEVICE_WIFI_NAME)){
                    tmpList.add(wifiBean);
                }
            }
            for (int a=0;a< tmpList.size();a++){
                if (nowList.contains(tmpList.get(a))){
                    nowList.remove(tmpList.get(a));
                }
                realWifiList.add(0,tmpList.get(a));
            }
            try {
                Thread.sleep(200);
                for (int b =0 ;b< nowList.size();b++){
                    realWifiList.add(nowList.get(b));
                }
    ...
    public void wifiListSet(String wifiName, int type) {
        ...
        tmpList.clear();
        for (int i=0;i<realWifiList.size();i++){
               if (realWifiList.get(i).getWifiName().startsWith(DEVICE_WIFI_NAME)){
                   tmpList.add(realWifiList.get(i));
                   realWifiList.remove(realWifiList.get(i));
               }
        }
        try {
            Thread.sleep(200);
            for (int j=0;j<tmpList.size();j++){
                realWifiList.add(0,tmpList.get(j));
            }
        } catch (InterruptedException e) {
            e.printStackTrace();
        }
        ...
        for (int c=0;c< realWifiList.size();c++){
            Log.e(TAG,"wifi名称---- 333 ： "+realWifiList.get(c).getWifiName());
        }
    }
